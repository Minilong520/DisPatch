...
import { getLoginUser } from './services/request/userinfo';
...
export async function getInitialState(): Promise<{
  ...
  const fetchUserInfo = async () => {
    try { const msg = await getLoginUser(); return msg.data; } catch (error) { history.push(loginPath); }
    return undefined;
  };
...
export const layout: RunTimeLayoutConfig = ({ initialState, setInitialState }) => {
  return {
    ...
    waterMarkProps: { content: "DisPatch Project" },
    //footerRender: () => <Footer />,
    onPageChange: () => {
      const { location } = history;
      // 如果没有登录，重定向到 login
